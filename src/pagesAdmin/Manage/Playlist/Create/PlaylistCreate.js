...
function PlaylistCreate() {
    ...
    const handleCreatePlaylist = async () => {
        try {
            ...
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/playlist/create`, formData);
    
            if(res.data.success) {
                dispatch(noticeAdminSlice.actions.successNotice('Thêm playlist thành công'));
        ...
        <div className='pb-[10px]'>
            <div className="py-[16px] px-[60px] bg-[#F7F9FA] flex items-center justify-between">
                <div className="flex items-center gap-[20px]">
                    <Link to={config.routes.adminPlaylist} className="text-[26px] h-[50px] text-[#858080] w-[50px] box-shadow-admin-path rounded-[4px] bg-[#fff] flex items-center justify-center">
                        <BsMusicNoteList />
                    </Link>
                    <h1 className="text-[20px] text-[#5a5757]">Playlist Create</h1>
                </div>
                <button
            ...
            <div className='m-[20px] p-[16px] bg-[#fff] rounded-[4px] box-shadow-admin-path'>
                    ...
                    <div className='flex gap-[20px]'>
                        <div className='flex-1'>
                            <div className='flex flex-col gap-[4px]'>
                                <label>Name Playlist: <span className='text-[red] text-[14px] font-[700]'>*</span></label>
                                <input ref={refName} value={name} name='name' onChange={(e) => setName(e.target.value)} className='border-[1px] px-[8px] py-[2px] outline-none border-solid border-[#a9a5a5] rounded-[4px]' type="text" placeholder="Name Playlist" />
                            </div>
                            <div className='mt-[14px]'>
                                <label>Image Playlist: <span className='text-[red] text-[14px] font-[700]'>*</span></label>
                                ...
                                <div className='mt-[6px] flex justify-center'>
                                    {
                                        ...
                                        <input value={image} name='name' onChange={(e) => setImage(e.target.value)} className='border-[1px] px-[8px] w-full py-[2px] outline-none border-solid border-[#a9a5a5] rounded-[4px]' type="text" placeholder="Link Image Playlist" />
                                    }
                                </div>
                            </div>
                            <div className='mt-[14px]'>
                ...
                <div className='flex justify-center mt-[30px]'>
                    <button disabled={loadingCreate || notice.state} onClick={handleCreatePlaylist} className={`${(loadingCreate || notice.state) ? 'opacity-[.7] hover:opacity-[.7]' : 'hover:opacity-[.9]'} w-[120px] h-[30px] flex justify-center items-center bg-[#259d23] rounded-[4px] text-[#fff] active:opacity-[.7]`}>
                        {
