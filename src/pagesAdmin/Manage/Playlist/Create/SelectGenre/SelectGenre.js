...
function SelectGenre({ listIdGenre, setListIdGenre, listSelect, setListSelect }) {
    ...
    const getDate = async () => {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/v1/genre/getAll?search=${search}`);
    ...
    const handleDragEnd = (e) => {
        ...
        if (active.id !== over.id) {
            ...
            const oldIndex = genres.findIndex((c) => c._id === active.id);

            // Lấy vị trí mới từ over
            const newIndex = genres.findIndex((c) => c._id === over.id);

            const dndGenres = arrayMove(genres, oldIndex, newIndex);
            setGenres(dndGenres);
        }

        if('310702' === over?.id && !listIdGenre.includes(active.id) && active?.id !== '310702') {
            setListSelect(prev => [...prev, active?.data?.current]);
            setListIdGenre(prev => [...prev, active?.id]);
        }
    ...
    const handleAddCardSelect = (genre) => {
        if(!listIdGenre.includes(genre._id)) {
            setListSelect(prev => [...prev, genre]);
            setListIdGenre(prev => [...prev, genre?._id]);
        }
    }
    ...
    const handleDeleteCardSelect = (id) => {
        const listIdGenreNew = listIdGenre.filter(item => item !== id);
    ...
    return (
        <DndContext sensors={mySensors} onDragEnd={handleDragEnd} onDragStart={handleDragStart} onDragMove={handleDragMove}>
            <div className="flex mt-[20px] flex-col">
                <div className="flex flex-col">
                    <label>
                        Chọn thể loại: <span className="text-[red] text-[14px] font-[700]">*</span>
                    </label>
                    <div className='max-h-[300px] overflow-y-auto scrollbar-admin-sidebar'>
                        <div className="px-[16px] py-[10px] flex flex-wrap gap-[10px] mt-[4px] min-h-[50px]">
                            {
                                listSelect?.map((genre, index) => {
                                    return <CardSelect key={index} genre={genre} handleDeleteCardSelect={handleDeleteCardSelect} />
                                })
                            }
                ...
                <div className="flex flex-col mt-[20px]">
                    <div className="flex flex-col items-start">
                        <label className='capitalize font-[600]'>Thể loại gợi ý</label>
                    ...
                    <div className="px-[16px] py-[10px] flex flex-wrap gap-[10px] mt-[10px] max-h-[300px] overflow-y-auto scrollbar-admin-sidebar">
                        <SortableContext items={genres?.map((c) => c._id)} strategy={rectSortingStrategy}>
                            {
                                genres.length !== 0 ?
                                genres?.map((genre, index) => {
                                    return <Card key={index} genre={genre} handleAddCardSelect={handleAddCardSelect} />
                                }) :
                                <div className='flex justify-center w-full font-[600] text-[13px] uppercase text-[#af9f9f]'>Không có thể loại này trong danh sách</div>
                            }
                        </SortableContext>
                    </div>
                </div>
