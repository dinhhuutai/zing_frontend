...
import { BsPersonPlus, BsStarFill, BsArrowRepeat } from 'react-icons/bs';
...
function PartnerUpdate() {
    ...
    const handleUpdate = async () => {
        try {
            if(!name){
                dispatch(noticeAdminSlice.actions.errorNotice('Vui lòng nhập tên đối tác'));
                ...
                dispatch(noticeAdminSlice.actions.errorNotice('Vui lòng thêm hình ảnh đối tác'));
            ...
            dispatch(noticeAdminSlice.actions.processingNotice('Đang Cập nhật đối tác'));
            ...
            const res = await axios.put(`${process.env.REACT_APP_API_URL}/v1/partner/update/${id}`, formData);
    
            if(res.data.success) {
                dispatch(noticeAdminSlice.actions.successNotice('Cập nhật đối tác thành công'));
    ...
    const getData = async () => {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/v1/partner/getSingle/${id}`);
        ...
        <div>
            <div className="py-[16px] px-[60px] bg-[#F7F9FA] flex items-center justify-between">
                <div className="flex items-center gap-[20px]">
                    <Link to={config.routes.adminPartner} className="text-[26px] h-[50px] text-[#858080] w-[50px] box-shadow-admin-path rounded-[4px] bg-[#fff] flex items-center justify-center">
                        <BsPersonPlus />
                    </Link>
            ...
            <div className='m-[20px] p-[16px] bg-[#fff] rounded-[4px] box-shadow-admin-path'>
                <div className='flex gap-[20px]'>
                    <div className='flex-1'>
                        <div className='flex flex-col gap-[4px]'>
                            <label>Name Partner: <span className='text-[red] text-[14px] font-[700]'>*</span></label>
                            <input ref={refName} value={name} name='name' onChange={(e) => setName(e.target.value)} className='border-[1px] px-[8px] py-[2px] outline-none border-solid border-[#a9a5a5] rounded-[4px]' type="text" placeholder="Name Partner" />
                        </div>
                        <div className='mt-[14px]'>
                            <label>Image Partner: <span className='text-[red] text-[14px] font-[700]'>*</span></label>
                            ...
                            <div className='mt-[6px] flex justify-center'>
                                {
                                    ...
                                    <div className='relative w-[210px] h-[210px] border-[2px] border-dashed border-[#989494] cursor-pointer bg-[#fff]'>
                                        ...
                                        <div className='absolute top-0 left-0 w-full h-full text-[#877575] flex flex-col items-center justify-center text-[14px]'>
                                            ...
                                            <p className='mt-[20px]'>Upload a partner image</p>
                                        </div>
                                    ...
                                    <input value={image} name='name' onChange={(e) => setImage(e.target.value)} className='border-[1px] px-[8px] w-full py-[2px] outline-none border-solid border-[#a9a5a5] rounded-[4px]' type="text" placeholder="Link Image Partner" />
                                }
                            </div>
                        </div>
                    </div>
                ...
                <div className='flex justify-center mt-[30px]'>
                    <Alert funcHandle={handleUpdate} title='Update Partner' content={`Bạn có chắc chắn muốn cập nhật lại đối tác ${name} không?`}>
