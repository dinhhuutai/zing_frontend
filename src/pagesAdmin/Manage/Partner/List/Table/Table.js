...
function Table({
    ...
    useEffect(() => {
        ...
        if(newArray.length === partners.length && partners.length !== 0) {
                        ...
                        return (
                            ...
                            >
                                <td className="w-[30px] border-r-solid border-r-[1px] border-r-[#c4b8b8] flex justify-center items-center">
                                    <input checked={listCheck.includes(index)} onChange={(e) => handleCheckItem(index, e)} data-id={listCheck.includes(index) ? partner._id : null} name="inputCheckboxArtistList" type="checkbox" />
                                </td>
                                ...
                                <td className='w-[200px]'>
                                    <Link to={`${config.routes.adminPartnerUpdate}/${partner._id}`} className="w-full h-full cursor-pointer border-r-solid border-r-[1px] border-r-[#c4b8b8] flex items-center px-[10px] py-[4px]">
                                        {partner.name}
                                    </Link>
                                </td>
                                <td className="w-[100px] h-[100px] border-r-solid border-r-[1px] border-r-[#c4b8b8] flex items-center px-[10px] py-[10px]">
                                    <img
                                        className="rounded-[4px] w-full h-full object-contain border-[1px] border-[#b9b0b0] border-solid"
                                        alt={partner.name}
                                        src={partner.image}
                                    />
                                </td>
                                <td className="w-[150px] border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">{`${new Date(
                                    partner.createDate,
                                ).getDate()}/${new Date(partner.createDate).getMonth() + 1}/${new Date(
                                    partner.createDate,
                                ).getFullYear()}`}</td>
                                <td className="w-[180px] flex items-center px-[10px] py-[4px]">{`${new Date(
                                    partner.updateDate,
                                ).getDate()}/${new Date(partner.updateDate).getMonth() + 1}/${new Date(
                                    partner.updateDate,
                                ).getFullYear()}`}</td>
                            </tr>
                        );
                    }) :
                    <div className='flex mt-[20px] justify-center w-full font-[600] text-[13px] uppercase text-[#ae9c9c]'>Không có đối tác này trong danh sách</div>
                }
                </tbody>
            </table>
        </div>
