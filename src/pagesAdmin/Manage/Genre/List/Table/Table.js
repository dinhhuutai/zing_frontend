...
function Table({
    ...
    useEffect(() => {
        ...
        if(newArray.length === genres.length && genres.length !== 0) {
                        ...
                        return (
                            ...
                            >
                                <td className="w-[30px] border-r-solid border-r-[1px] border-r-[#c4b8b8] flex justify-center items-center">
                                    <input checked={listCheck.includes(index)} onChange={(e) => handleCheckItem(index, e)} data-id={listCheck.includes(index) ? genre._id : null} name="inputCheckboxArtistList" type="checkbox" />
                                </td>
                                ...
                                <td className='w-[200px]'>
                                    <Link to={`${config.routes.adminGenreUpdate}/${genre._id}`} className="w-full h-full cursor-pointer border-r-solid border-r-[1px] border-r-[#c4b8b8] flex items-center px-[10px] py-[4px]">
                                        {genre.name}
                                    </Link>
                                </td>
                                ...
                                <td className="w-[150px] border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">{`${new Date(
                                    genre.createDate,
                                ).getDate()}/${new Date(genre.createDate).getMonth() + 1}/${new Date(
                                    genre.createDate,
                                ).getFullYear()}`}</td>
                                <td className="w-[180px] flex items-center px-[10px] py-[4px]">{`${new Date(
                                    genre.updateDate,
                                ).getDate()}/${new Date(genre.updateDate).getMonth() + 1}/${new Date(
                                    genre.updateDate,
                                ).getFullYear()}`}</td>
                            </tr>
                        );
                    }) :
                    <div className='flex mt-[20px] justify-center w-full font-[600] text-[13px] uppercase text-[#ae9c9c]'>Không có thể loại này trong danh sách</div>
                }
                </tbody>
            </table>
        </div>
