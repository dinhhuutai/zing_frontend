...
function Table({
    ...
    useEffect(() => {
        ...
        if(newArray.length === users.length && users.length !== 0) {
            ...
            <table className="w-full flex flex-col justify-center">
                <thead className="w-full sticky top-[var(--admin-height-header)] bg-[#fff]">
                    <tr className="flex border-[1px] border-solid border-[#c4b8b8] rounded-t-[4px]">
                        ...
                        <th className="w-[150px] flex-1 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] py-[4px] flex items-center justify-start">
                            Username
                        </th>
                        <th className="w-[50px] flex-1 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] py-[4px] flex items-center justify-start">
                            Admin
                        </th>
                        <th>
                            <button
                        ...
                        return (
                            ...
                            >
                                <td className="w-[30px] border-r-solid border-r-[1px] border-r-[#c4b8b8] flex justify-center items-center">
                                    <input checked={listCheck.includes(index)} onChange={(e) => handleCheckItem(index, e)} data-id={listCheck.includes(index) ? user._id : null} name="inputCheckboxArtistList" type="checkbox" />
                                </td>
                                ...
                                <td className='w-[200px]'>
                                    <Link to={`${config.routes.adminUserUpdate}/${user._id}`} className="w-full h-full cursor-pointer border-r-solid border-r-[1px] border-r-[#c4b8b8] flex items-center px-[10px] py-[4px]">
                                        {user.name}
                                    </Link>
                                </td>
                                ...
                                <td className="w-[150px] flex-1 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">
                                    {
                                        user.username
                                    }
                                </td>
                                <td className="w-[50px] flex-1 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">
                                    {
                                ...
                                <td className="w-[150px] border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">{`${new Date(
                                    user.createDate,
                                ).getDate()}/${new Date(user.createDate).getMonth() + 1}/${new Date(
                                    user.createDate,
                                ).getFullYear()}`}</td>
                                <td className="w-[180px] flex items-center px-[10px] py-[4px]">{`${new Date(
                                    user.updateDate,
                                ).getDate()}/${new Date(user.updateDate).getMonth() + 1}/${new Date(
                                    user.updateDate,
                                ).getFullYear()}`}</td>
                            </tr>
                        );
                    }) :
                    <div className='flex mt-[20px] justify-center w-full font-[600] text-[13px] uppercase text-[#ae9c9c]'>Không có ca sĩ này trong danh sách</div>
                }
                </tbody>
            </table>
        </div>
