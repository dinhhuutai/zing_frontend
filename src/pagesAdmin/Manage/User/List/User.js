...
function User() {
    ...
    const handleDelete = async () => {
        ...
        dispatch(noticeAdminSlice.actions.processingNotice('Đang xóa user'));

        const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/user/delete`, {listId: listIdDelete});

        if(res.data.success){

            dispatch(noticeAdminSlice.actions.successNotice('Xóa user thành công'));
            setListCheck([]);
        ...
        <div className="pb-[10px]">
            <div className="py-[16px] px-[60px] bg-[#F7F9FA] flex items-center justify-between">
                <div className="flex items-center gap-[20px]">
                    ...
                    <h1 className="text-[20px] text-[#5a5757]">User List</h1>
                </div>
