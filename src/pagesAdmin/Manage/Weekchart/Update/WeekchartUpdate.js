...
function PartnerUpdate() {
    ...
    const handleUpdate = async () => {
        try {
            ...
            dispatch(noticeAdminSlice.actions.processingNotice('Đang Cập nhật weekchart'));
            ...
            const res = await axios.put(`${process.env.REACT_APP_API_URL}/v1/weekchart/update/${id}`, formData);
    
            if(res.data.success) {
                dispatch(noticeAdminSlice.actions.successNotice('Cập nhật weekchart thành công'));
    ...
    const getData = async () => {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/v1/weekchart/getSingle/${id}`);

        if(res.data.success) {
            setName(res.data.weekchart.name);
            setImage(res.data.weekchart.image);
        ...
        <div>
            ...
            <div className='m-[20px] p-[16px] bg-[#fff] rounded-[4px] box-shadow-admin-path'>
                ...
                <div className='flex justify-center mt-[30px]'>
                    <Alert funcHandle={handleUpdate} title='Update Weekchart' content={`Bạn có chắc chắn muốn cập nhật lại weekchart ${name} không?`}>
