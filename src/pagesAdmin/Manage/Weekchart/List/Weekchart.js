...
import { BsBarChartLine, BsStarFill, BsPlusSquare } from 'react-icons/bs';
...
function WeekChart() {
    ...
    const handleDelete = async () => {
        ...
        dispatch(noticeAdminSlice.actions.processingNotice('Đang xóa weekchart'));

        const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/weekchart/delete`, {listId: listIdDelete});

        if(res.data.success){

            dispatch(noticeAdminSlice.actions.successNotice('Xóa weekchart thành công'));
            setListCheck([]);
    ...
    const handleGetData = async () => {
        try {
            const res = await axios.post(
                `${process.env.REACT_APP_API_URL}/v1/weekchart/find?limit=${show}&skip=${
                    show * currentPage
                }&search=${search}&${
            ...
            if (res.data.success) {
                setData({
                    ...data,
                    weekchart: res.data.weekchart,
                    totalWeekchart: res.data.totalWeekchart,
        ...
        <div className="pb-[10px]">
                ...
                <div className="flex gap-[16px]">
                    <button>
                        <Link
                            ...
                            to={config.routes.adminWeekchartCreate}
                        >
            ...
            <div className="m-[20px] p-[16px] bg-[#fff] rounded-[4px] box-shadow-admin-path">
                ...
                <BottomTableAdmin
                    ...
                    show={show}
                    currentPage={currentPage}
                    totalItems={data.totalWeekchart}
                />
            </div>
        </div>
    );
