...
function Table({
    ...
    useEffect(() => {
        ...
        if(newArray?.length === albums?.length && albums?.length !== 0) {
                        ...
                        return (
                            ...
                            >
                                <td className="w-[30px] shrink-0 border-r-solid border-r-[1px] border-r-[#c4b8b8] flex justify-center items-center">
                                    <input checked={listCheck.includes(index)} onChange={(e) => handleCheckItem(index, e)} data-id={listCheck.includes(index) ? album._id : null} name="inputCheckboxArtistList" type="checkbox" />
                                </td>
                                ...
                                <td className='w-[200px] shrink-0'>
                                    <Link to={`${config.routes.adminAlbumUpdate}/${album._id}`} className="w-full h-full cursor-pointer border-r-solid border-r-[1px] border-r-[#c4b8b8] flex items-center px-[10px] py-[4px]">
                                        {album.name}
                                    </Link>
                                </td>
                                ...
                                <td className="w-[150px] shrink-0 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">{`${new Date(
                                    album.releaseDate*1000,
                                ).getDate()}/${new Date(album.releaseDate*1000).getMonth() + 1}/${new Date(
                                    album.releaseDate*1000,
                                ).getFullYear()}`}</td>
                                <td className="w-[150px] shrink-0 border-r-solid border-r-[1px] border-r-[#c4b8b8] px-[10px] flex items-center py-[4px]">{`${new Date(
                                    album.createDate,
                                ).getDate()}/${new Date(album.createDate).getMonth() + 1}/${new Date(
                                    album.createDate,
                                ).getFullYear()}`}</td>
                                <td className="w-[180px] shrink-0 flex items-center px-[10px] py-[4px]">{`${new Date(
                                    album.contentLastUpdate,
                                ).getDate()}/${new Date(album.contentLastUpdate).getMonth() + 1}/${new Date(
                                    album.contentLastUpdate,
                                ).getFullYear()}`}</td>
                            </tr>
                        );
                    }) :
                    <div className='flex mt-[20px] justify-center w-full font-[600] text-[13px] uppercase text-[#ae9c9c]'>Không có album này trong danh sách</div>
                }
                </tbody>
            </table>
        </div>
