...
function Card({song, handleAddCardSelect}) {

    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: song?._id, data: {...song} });
    ...
    return (
        ...
        >
            ...
            <button onClick={() => handleAddCardSelect(song)} className="ml-[6px] bg-[#fff] p-[4px] rounded-[50%] box-shadow-admin-notice hover:opacity-[.8] active:opacity-[.7]">
                <BsPlus />
            </button>
        </button>
    );
