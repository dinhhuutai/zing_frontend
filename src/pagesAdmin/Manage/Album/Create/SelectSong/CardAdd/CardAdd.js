...
function CardAdd({activeCardAdd}) {
    ...
    return (
        <button
            ...
            {...attributes}
            {...listeners}
            className={`${activeCardAdd ? 'box-shadow-admin-card-add' : ''} px-[6px] py-[4px] rounded-[20px] bg-[#fff] w-full flex items-center justify-center cursor-pointer box-shadow-admin-path`}
        >
