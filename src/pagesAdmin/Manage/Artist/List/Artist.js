...
import { BsMic, BsStarFill, BsPlusSquare } from 'react-icons/bs';
...
function Artist() {
    ...
    const handleDelete = async () => {
        ...
        dispatch(noticeAdminSlice.actions.processingNotice('Đang xóa artist'));

        const res = await axios.post(`${process.env.REACT_APP_API_URL}/v1/artist/delete`, {listId: listIdDelete});

        if(res.data.success){

            dispatch(noticeAdminSlice.actions.successNotice('Xóa nghệ sĩ thành công'));
            setListCheck([]);
    ...
    const handleGetData = async () => {
        try {
            const res = await axios.post(
                `${process.env.REACT_APP_API_URL}/v1/artist/find?limit=${show}&skip=${
                    show * currentPage
                }&search=${search}&${
            ...
            if (res.data.success) {
                setData({
                    ...
                    totalArtist: res.data.totalArtist,
        ...
        <div className="pb-[10px]">
            <div className="py-[16px] px-[60px] bg-[#F7F9FA] flex items-center justify-between">
                <div className="flex items-center gap-[20px]">
                    ...
                    <h1 className="text-[20px] text-[#5a5757]">Artist List</h1>
                </div>
                <div className="flex gap-[16px]">
                    <button>
                        <Link
                            ...
                            to={config.routes.adminArtistCreate}
                        >
