...
function ArtistUpdate() {
    ...
    const handleUpdateArtist = async () => {
        try {
            ...
            dispatch(noticeAdminSlice.actions.processingNotice('Đang Cập nhật artist'));
            ...
            const res = await axios.put(`${process.env.REACT_APP_API_URL}/v1/artist/update/${id}`, formData);
    
            if(res.data.success) {
                dispatch(noticeAdminSlice.actions.successNotice('Cập nhật nghệ sĩ thành công'));
    ...
    const getData = async () => {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/v1/artist/getSingle/${id}`);

        if(res.data.success) {
            setName(res.data.artist.name);
            setInformation(res.data.artist.information);
            setImage(res.data.artist.image);
        ...
        <div>
            ...
            <div className='m-[20px] p-[16px] bg-[#fff] rounded-[4px] box-shadow-admin-path'>
                ...
                <div className='flex justify-center mt-[30px]'>
                    <Alert funcHandle={handleUpdateArtist} title='Update Artist' content={`Bạn có chắc chắn muốn cập nhật lại artist ${name} không?`}>
