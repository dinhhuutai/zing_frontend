    ...
    return (
        <div>
            <div className='relative group cursor-pointer'>
                <div className='overflow-hidden aspect-square rounded-[50%]'>
                    <img className="group-hover:scale-[1.1] h-full w-full object-cover group-hover:brightness-50 bg-cover transition-all ease-linear duration-[.4s]" alt={data?.name} src={data?.image} />
                </div>
            ...
            <div className="flex flex-col items-center mt-[10px]">
                ...
                <span className="text-[12px] cursor-default text-[hsla(0,0%,100%,0.5)]">{Math.floor(data?.following?.items?.length / 1000)}K quan tâm</span>
            </div>
